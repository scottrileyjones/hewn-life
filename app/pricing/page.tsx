  ...
  return (
    <>
      ...
      <section id="plans" ref={plansRef} className="bg-bone py-24 md:py-32">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-14">
            ...
            <p className="font-display font-semibold italic text-[22px] md:text-[28px] text-ink mt-5 mb-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            {tiers.map((tier, i) => {
              ...
              const cardBorder = isGold
                ? 'border-2 border-[#C9A84C]/50 shadow-2xl shadow-[#C9A84C]/15'
                : isSilver
                  ? 'border border-ember shadow-2xl shadow-ember/10'
                  : 'border border-black/10'

              return (
                <FadeInWrapper key={tier.slug} delay={i * 110} className="flex flex-col">
                  <div className={`rounded-2xl overflow-hidden ${cardBorder} ${isGold ? 'lg:-mx-2' : ''} flex flex-col h-full`}>
                    {/* Card header */}
                      ...
                      {tier.badge && (
                        <div className="absolute top-4 right-4">
                          <span
                            ...
                            style={isGold
                              ? { background: `${tier.accentColor}22`, color: tier.accentColor, border: `1px solid ${tier.accentColor}44` }
                              : { background: '#4A5348', color: '#F2ECE0' }
                            }
                          >
                            {tier.badge}
                          </span>
                    ...
                    <div className="bg-white p-8 flex flex-col flex-1">
                        ...
                        {annual ? (
                          <>
                            <div className="flex items-baseline gap-2 mb-1">
                              <span className="font-body text-sm text-slate/60 line-through">${tier.price.monthly.toLocaleString()}</span>
                              <span className="font-mono text-[10px] uppercase tracking-widest text-ember">Save 10%</span>
                            </div>
                            <div className="flex items-end gap-1">
                              <span className="font-display font-bold text-[52px] leading-none text-ink">${price.toLocaleString()}</span>
                              <span className="font-body text-base text-slate mb-1.5">/mo, billed annually</span>
                            </div>
                          </>
                        ) : (
                          <div className="flex items-end gap-1">
                      ...
                      <div className="mt-auto">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 font-body text-xs text-slate hover:text-ink transition-colors w-full"
                        >
      ...
      <div className="bg-bone border-t border-b border-black/[0.08] py-5 px-6">
        <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-slate text-center">
